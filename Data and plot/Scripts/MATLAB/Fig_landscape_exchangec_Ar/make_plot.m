...
function processFoldersAndPlotData(parentFolder, dataFile, zColumn, polygonVertices)
    ...
    'blank', 'Volt', 'Time', 'Exchange Current Density (mA/cm²)'});
    ...
    caxis([-7, 5]); % Set the color bar limits
    c = colorbar;
    configureColorBar(c, sprintf('Log(i_0 (mA/cm^2))')); % Font sizes in point
...
function m = metadata()
    m.stable_id = "Fig_landscape_exchangec_Ar";
    m.script = mfilename('fullpath');
    m.data_keys = ["All_Ovp_Cdl_Tafel_Ar_MATLAB"];
    m.figure_type = "SI";
end
