...
function processFoldersAndPlotData(parentFolder, dataFile, zColumn, polygonVertices)
    ...
    'blank', 'Volt', 'Time', 'Tafel Slope (mV/dec)'});
    ...
    caxis([20, 240]); % Set the color bar limits
    c = colorbar;
    configureColorBar(c, sprintf('Tafel Slope (mV/dec)')); % Font sizes in point
    colormap(gray);
...
function m = metadata()
    m.stable_id = "Fig_landscape_tafelslope_Ar";
