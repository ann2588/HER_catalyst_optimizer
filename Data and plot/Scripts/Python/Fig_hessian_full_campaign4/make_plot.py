...
FIGURE_METADATA = {
    "stable_id": "Fig_hessian_full_campaign4",
    "script": __file__,
    "data_keys": "In folder",
...
pd.concat([pd.read_csv(f"{CSV_FILENAME_1}"), pd.read_csv(f"{SCRIPT_DIR}/Set 2/Campaign4_stage1_only.csv")]).to_csv(CSV_FILENAME_2, index=False)
...
pd.concat([pd.read_csv(f"{CSV_FILENAME_2}"), pd.read_csv(f"{SCRIPT_DIR}/Set 2/Campaign4_stage2_only.csv")]).to_csv(CSV_FILENAME_3, index=False)
