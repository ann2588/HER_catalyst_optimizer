...
FIGURE_METADATA = {
    "stable_id": "Fig_interaction_rank_campaign1",
    "script": __file__,
    "data_keys": "In folder",
...
pd.concat([pd.read_csv(f"{CSV_FILENAME_1}"), pd.read_csv(f"{SCRIPT_DIR}/Set 2/Campaign1_stage1_only.csv")]).to_csv(CSV_FILENAME_2, index=False)
...
pd.concat([pd.read_csv(f"{CSV_FILENAME_2}"), pd.read_csv(f"{SCRIPT_DIR}/Set 2/Campaign1_stage2_only.csv")]).to_csv(CSV_FILENAME_3, index=False)
...
fig, ax = plot_three_top10_with_sankey_links(H_DOE_120, H_STAGE_1, H_STAGE_2, FEATURES, save_path = f'{OUTPUT_DIR}/Campaign1sanki.eps')
fig, ax = plot_three_top10_with_sankey_links(H_DOE_120, H_STAGE_1, H_STAGE_2, FEATURES, save_path = f'{OUTPUT_DIR}/Campaign1sanki.png')